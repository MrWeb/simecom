...
class VideoService
{
    ...
    public function generateDynamicOfferVideo(string $offerName, string $videoType): string
    {
        ...
        // Acquisisci lock PRIMA di qualsiasi check sul file per evitare race condition
        ...
        try {
            ...
            // Ora che abbiamo il lock esclusivo, controlliamo il file
            if (file_exists($outputPath)) {
                $fileSize = filesize($outputPath);

                // File valido (> 100KB per un video dynamic di ~7 secondi)
                if ($fileSize > 100000) {
                ...
                // File troppo piccolo = corrotto, elimina e rigenera
                Log::warning('Dynamic offer file corrupted, regenerating', [
                    'output' => $outputPath,
                    'file_size' => $fileSize,
                    'text' => $offerName,
                ]);
            ...
            // Verifica che la generazione sia andata a buon fine
            if (!file_exists($outputPath) || filesize($outputPath) < 100000) {
                $size = file_exists($outputPath) ? filesize($outputPath) : 0;
                throw new \RuntimeException("Dynamic offer generation failed: output file is {$size} bytes");
            }
